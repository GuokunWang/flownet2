...
#seq_name = "eyeglasses/"
    ...
    f = open('1.txt','w')
    for iterator in range(flow_gap, seq_len + 1,flow_gap):
        i = 1
        while i<=5:
            i+=1
        ...
        img0 = img1
        input_data = []
        img1 = cv2.imread(seq_path_prefix + seq_name + 'img1/'+  str(start_index + iterator + flow_gap).zfill(zero_numbers) + '.jpg' )
        ...
        for key in  current_position.keys():
            ...
            #caluclate location
            for flow_index in range(flow_gap - 1):
                actual_index = iterator - flow_gap + 1  + flow_index
                print(actual_index)
                output[key][actual_index + start_index] = [cx - bb_w/2, cy - bb_h/2,bb_w,bb_h]
            new_cx = round(cx + drift_x )
            new_cy = round(cy + drift_y )
            output[key][actual_index + 1 + start_index] = [new_cx - bb_w/2, new_cy - bb_h/2,bb_w,bb_h]
            #new_cx = round(cx + drift_x)
            #new_cy = round(cy + drift_y)
